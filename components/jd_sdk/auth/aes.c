...
static int AesSetKey(Aes* aes, const uint8_t* userKey, uint32_t keylen, const uint8_t* iv, int dir);
static int AesCbcEncrypt(Aes* aes, uint8_t* out, const uint8_t* in, uint32_t sz);
static int AesCbcDecrypt(Aes* aes, uint8_t* out, const uint8_t* in, uint32_t sz);

#define XMEMCPY(d,s,l)    memcpy((d),((void*)(s)),(l))

static const uint32_t Te[5][256] = {
{
...
static const uint32_t Td[5][256] = {
{
...
#define GETBYTE(x, y) (uint32_t)((uint8_t)((x) >> (8 * (y))))
...
/*STATIC INLINE */uint32_t rotrFixed(uint32_t x, uint32_t y)
{
	return (x >> y) | (x << (sizeof(y) * 8 - y));
}
...
/*STATIC INLINE */void ByteReverseWords(uint32_t* out, const uint32_t* in,
                                    uint32_t byteCount)
{
    uint32_t count = byteCount/sizeof(uint32_t), i;
...
/*STATIC INLINE */void XorWords(uint32_t* r, const uint32_t* a, uint32_t n)
{
    uint32_t i;
...
{
    if (((uint32_t)buf | (uint32_t)mask | count) % CYASSL_WORD_SIZE == 0)
        XorWords( (uint32_t*)buf, (const uint32_t*)mask, count / CYASSL_WORD_SIZE);
    else {
        uint32_t i;
...
static int AesSetIV(Aes* aes, const uint8_t* iv)
{
    if (aes == NULL)
        return BAD_FUNC_ARG;
...
static int AesSetKeyLocal(Aes* aes, const uint8_t* userKey, uint32_t keylen,
            const uint8_t* iv, int dir)
{
    uint32_t temp, *rk = aes->key;
    unsigned int i = 0;
...
int AesSetKey(Aes* aes, const uint8_t* userKey, uint32_t keylen, const uint8_t* iv,
              int dir)
{
...
static void AesEncrypt(Aes* aes, const uint8_t* inBlock, uint8_t* outBlock)
{
    uint32_t s0, s1, s2, s3;
    uint32_t t0, t1, t2, t3;
    uint32_t r = aes->rounds >> 1;

    const uint32_t* rk = aes->key;
    if (r > 7 || r == 0) {
        //printf("AesEncrypt encountered improper key, set it up");
...
static void AesDecrypt(Aes* aes, const uint8_t* inBlock, uint8_t* outBlock)
{
    uint32_t s0, s1, s2, s3;
    uint32_t t0, t1, t2, t3;
    uint32_t r = aes->rounds >> 1;
    ...
    const uint32_t* rk = aes->key;
    if (r > 7 || r == 0) {
        //printf("AesDecrypt encountered improper key, set it up");
        return;  /* stop instead of segfaulting, set up your keys! */
    }
...
{
    uint32_t blocks = sz / AES_BLOCK_SIZE;

    while (blocks--) {
        xorbuf((uint8_t*)aes->reg, in, AES_BLOCK_SIZE);
        AesEncrypt(aes, (uint8_t*)aes->reg, (uint8_t*)aes->reg);
...
int AesCbcDecrypt(Aes* aes, uint8_t* out, const uint8_t* in, uint32_t sz)
{
    uint32_t blocks = sz / AES_BLOCK_SIZE;

    while (blocks--) {
        XMEMCPY(aes->tmp, in, AES_BLOCK_SIZE);
        AesDecrypt(aes, (uint8_t*)aes->tmp, out);
        xorbuf(out, (uint8_t*)aes->reg, AES_BLOCK_SIZE);
...
uint8_t  AES_iv[] = "0123456789abcdef";  /* align */
int device_aes_encrypt(const uint8_t * key, int keyLength, const uint8_t * iv, const uint8_t *pPlainIn, int plainLength, uint8_t *pEncOut, int maxOutLen)
{
	int  i;
	Aes enc;
	int dat_len=plainLength;
	AesSetKey(&enc, key, keyLength, iv, AES_ENCRYPTION);
	if (dat_len!= (dat_len/AES_BLOCK_SIZE)*AES_BLOCK_SIZE)
		dat_len= (dat_len/AES_BLOCK_SIZE+1)*AES_BLOCK_SIZE;
	if (maxOutLen<dat_len)
	{
		return -1;
	}
	memcpy(pEncOut, pPlainIn, plainLength);
	if((dat_len-plainLength)>0)
	{
		for(i=plainLength;i<dat_len;i++)
		{
			pEncOut[i] = dat_len - plainLength;
		}
	}
	else if((dat_len-plainLength)==0)
	{
		dat_len=dat_len+0x10;
		for(i=plainLength;i<dat_len+16;i++)
		{
			pEncOut[i] = 0x10;
		}
	}
	AesCbcEncrypt(&enc, (uint8_t *)pEncOut, pEncOut, dat_len);
	return dat_len;
}
...
int device_aes_decrypt(const uint8_t * key, int keyLength, const uint8_t * iv, const uint8_t *pEncIn, int encLength, uint8_t *pPlainOut, int maxOutLen)
{
	...
	int dat_len= encLength;
	Aes dec;
	AesSetKey(&dec, key, keyLength, iv, AES_DECRYPTION);
	AesCbcDecrypt(&dec, (uint8_t *)pPlainOut, pEncIn, dat_len);
	dat_len = dat_len - pPlainOut[dat_len - 1];
	return dat_len;
}
